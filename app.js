...
(() => {
  ...
  function resetProjectState() {
    ...
    state.compilerHydrated = false; state.compilerHydrating = false; state.compilerHydrationPromise = null; state.compileSerial++;
  ...
  async function hydrateCompilerFS(force = false) {
    ...
    const hydration = (async () => {
      ...
      try {
        ...
        const total = entries.reduce((sum, [, meta]) => sum + (meta.size || 0), 0);
  ...
  function normalizePreviewSvg() {
    ...
    const svgs = Array.from(output.querySelectorAll(':scope > svg'));
    ...
    svgs.forEach((svg, index) => {
      ...
      if (w && h) {
        ...
        svg.dataset.baseHeight = h;
        svg.style.width = `${w}px`;
        svg.style.height = `${h}px`;
      }
      svg.dataset.pageNumber = String(index + 1);
      ...
      if (index < svgs.length - 1) {
        ...
        divider.className = 'preview-page-break';
        divider.setAttribute('aria-hidden', 'true');
        divider.innerHTML = `<span>Page ${index + 1}</span>`;
  ...
  function buildSourceSearchOrder() {
    const typPaths = effectivePaths().filter(path => ext(path) === 'typ');
    ...
    const visit = path => {
      ...
      const refs = [];
      const regex = /#(?:include|import)\s*(?:\(\s*)?["']([^"']+)["']/g;
      let match;
      while ((match = regex.exec(text))) refs.push(resolveTypstReference(path, match[1]));
      for (const ref of refs) visit(ref);
    };
    visit(state.mainPath);
    for (const path of typPaths) visit(path);
    for (const path of effectivePaths()) {
      if (!visited.has(path) && DATA_EXTENSIONS.has(ext(path)) && isTextPath(path)) order.push(path);
    }
    return order;
  }
  ...
  function indexPreviewSourceAnchors() {
    state.previewSourceAnchors = [];
    state.previewAnchorMap = new WeakMap();
